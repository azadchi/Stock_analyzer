...
MODEL_NAME = os.environ.get("BRS_MODEL", "gemini-3-flash")
...
def build_prompt(symbol, timeframe, notes, n_images):
    lines = [
        ...
        "وظیفه: تحلیل تکنیکال نماد زیر و صدور یک سیگنال شفاف.",
        "",
        "نماد: " + symbol,
        "تایم‌فریم: " + timeframe,
        "تعداد تصویر پیوست (نمودار / تابلو TSETMC): " + str(n_images),
    ]
    if notes:
...
def call_gemini(prompt, images):
    ...
    for attempt in range(1, GEMINI_MAX_RETRIES + 2):
        try:
            ...
            last_error = exc
            log.warning("خطای موقت Gemini (تلاش %d/%d): %s", attempt, GEMINI_MAX_RETRIES + 1, exc)
...
def analyze():
    ...
    prompt = build_prompt(symbol, timeframe, notes, len(images))
